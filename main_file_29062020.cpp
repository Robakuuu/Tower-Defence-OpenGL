...
bool createNewTower(int nameEnum) {
	...
	if (val <= GlobalProperties::coins) {
		GlobalProperties::coins -= val;
		towers.push_back(Tower(MacierzStolu, nameEnum));
		Logger::info("main_file", "Nowy Tower: " + to_string(nameEnum));
...
void createNewHero(int nameEnum) {
	//heros.push_back(Hero(heroSpeed, przypiszKlatke("kostka")));
	heros.push_back(Hero(przypiszKlatke("kostka"), nameEnum));
	Logger::info("main_file", "Nowy Hero: " + to_string(nameEnum));
}
...
void key_callback(GLFWwindow* window, int key,
	...
	if (action == GLFW_PRESS) {
		...
		if (isCreatingTower) {
			...
			Logger::info("TimerJobs", "glfwTime [" + to_string(glfwGetTime()) + "]");
			Logger::info("Wallet", "Masz: " + to_string(GlobalProperties::coins) + " golda");
		}
		else if (key == GLFW_KEY_ESCAPE && !isCreatingTower) {
...
int getNumerKlatki(string nazwaObiektu, int aktualnyNumerKlatki)
{
	for(vector<Obiekty >::iterator obiekt = WektorObiektowSwiata.begin(); obiekt != WektorObiektowSwiata.end(); ++obiekt)
	{
		...
		{
			if (aktualnyNumerKlatki >= obiekt->koniecAnimacji)
			{
				aktualnyNumerKlatki = obiekt->startAnimacji;
				
			}
			return aktualnyNumerKlatki;
...
void drawScene(GLFWwindow* window, float angle, int numerobiektu) {
	...
	for (vector<Hero>::iterator hero = heros.begin(); hero != heros.end(); ++hero)
	{
		...
		hero->aktualnaKlatkaAnimacji= getNumerKlatki("kostka", hero->aktualnaKlatkaAnimacji);
		rysujobiekt(hero->aktualnaKlatkaAnimacji++);
		//Dopisac vector animacji 
	}
...
void wczytaj_obiekty() {
	...
	doodczytu = readTexture("tekstury//Cube_Cube002.png");//stol
	wektortekstur.push_back(doodczytu);
	numerswiata++;
	przygotuj_obiektVer2("model//Drzewko.obj", numerobiektu);
	doodczytu = readTexture("tekstury//Cube_Cube002.png");//stol
	wektortekstur.push_back(doodczytu);
	numerswiata++;
	...
	doodczytu = readTexture("tekstury//Trawa.png");//stol
	wektortekstur.push_back(doodczytu);
	numerswiata++;
	...
	wektortekstur.push_back(doodczytu);
	numerswiata++;
	przygotuj_obiektVer2("model//Robocik//RobocikPrawyWalek.obj", numerobiektu);
	doodczytu = readTexture("tekstury//Plane022_Plane008.png");
	wektortekstur.push_back(doodczytu);
	numerswiata++;
	przygotuj_obiektVer2("model//Robocik//RobocikLewyWalek.obj", numerobiektu);
	doodczytu = readTexture("tekstury//Plane022_Plane008.png");
	wektortekstur.push_back(doodczytu);
	numerswiata++;
	przygotuj_obiektVer2("model//Robocik//RobocikTors.obj", numerobiektu);
	doodczytu = readTexture("tekstury//Plane022_Plane008.png");
	wektortekstur.push_back(doodczytu);
	numerswiata++;
	przygotuj_obiektVer2("model//Robocik//RobocikKola.obj", numerobiektu);
	doodczytu = readTexture("tekstury//Plane022_Plane008.png");
	wektortekstur.push_back(doodczytu);
	numerswiata++;
	przygotuj_obiektVer2("model//Robocik//RobocikPasek1.obj", numerobiektu);
	doodczytu = readTexture("tekstury//Plane022_Plane008.png");
	wektortekstur.push_back(doodczytu);
	numerswiata++;
	przygotuj_obiektVer2("model//Robocik//RobocikPasek2.obj", numerobiektu);
	doodczytu = readTexture("tekstury//Plane022_Plane008.png");
	wektortekstur.push_back(doodczytu);
	numerswiata++;
...
void wczytajAnimacje(string tekstura, string nazwaAnimacji)
{
	vector<Obiekty>animacja;
	const char* sciezkaTekstury=tekstura.c_str();
	for (int x = 0; x < sciezkiDoObiektow.size(); x++)
	{
		...
		doodczytu = readTexture(sciezkaTekstury);
		wektortekstur.push_back(doodczytu);
...
void wczytajObiektySwiata(int ostatniNumerSwiata)
{
	...
	wczytajAnimacje("tekstury//test.png", "kostka");
	sciezkiDoObiektow = czytaj_sciezke(sciezkiDoObiektow, "model//RobotRece");
	wczytajAnimacje("tekstury//test.png", "RobotRece");
